...
(i) Selecting programs to fund from a list of programs whose effectiveness is only partially observable, taking into account the inherent uncertainties in program outcomes and impact.
...
As previously mentioned, I'm going to use a simplified environment while trying to stay faithful to the most pertinent aspects of the real-world environment. The funder is assumed to be confronted with a set of programs, $\mathcal{K}$, and has to decide which program(s) to fund and which to re-evaluate.[$\mathcal{K} = \{1,\ldots,K\}$. In this study, $K = 10$.]{.aside} They will have to repeatedly revisit this decision over a number of _steps_.[In a health or development program, re-evaluation means collecting data from a randomized trial.]{.aside}

A good place to start is to model the environment as a multi-armed bandit (MAB). Each program/intervention is represented by such a bandit with a stochastic causal model: at every step in the sequential environment a new *state* -- drawn from a *hyperstate* -- determines the outcomes of the population targeted.[This is a broad simplification. In reality, we would distinguish between _programs_ and _populations_; different programs can be effective in different populations and different populations could be simultaneously targeted by a program.]{.aside} 
...
where $z$ is a binary variable indicating whether a program is implemented or not, which means $\tau_{kt}$ is average treatment effect. We therefore denote the state of a program to be $\boldsymbol{\theta}_{kt} = (\mu_{kt}, \tau_{kt}, \sigma_k)$. 
...
\end{align*}
$$ 
where $\boldsymbol{\xi} = (\xi^\mu, \xi^\tau, \xi^\sigma, \xi^{\eta^\mu}, \xi^{\eta^\tau})$ are the hyperparameters for the environment. This means that while each program has a fixed average baseline outcome, $\mu_k$, and average treatment effect, $\tau_k$, at every step, normally distributed shocks, with mean zero, alter the realized averages. [With some abuse of notation, I will write $\boldsymbol{\theta_{kt}\sim\theta_k}$ and $\boldsymbol{\theta_k\sim\boldsymbol{\xi}}$.]{.aside} 

In this simple environment the hierarchical structure represents the heterogeneity of program effectiveness over time, emphasizing the limitations of a single evaluation of a program at a particular point in time. I am making the simplifying assumption that this variation is purely an oscillation without any trends. Furthermore, agents should also be concerned about variations in effectiveness when programs are implemented in different contexts, but we ignore that in this environment assuming the time variation captures the general problem of heterogeneity over time and context.[Context here refers to geography or populations. Meta-analyses are typically aimed at understanding the generalizability of evaluations between contexts.]{.aside} Since states vary randomly and independently, the objective of the funder is to learn about the hyperstate rather than the next realized state.[Future iterations of this model could introduce some correlation between states over time.]{.aside} 
...
The funder is never aware of the true state of the world --- the true counterfactual model of all programs' effectiveness --- but they are able to evaluate a program by collecting experimental data and updating their beliefs. I assume that the funder has an initial observation (data from an evaluation) for each program under consideration. This could be data from an earlier experiment or could represent the agent's prior beliefs.
...
crossing(a = seq(0, 0.5, 0.125/2), c = seq(-4, 4, 0.1)) |> 
  ...
  NULL
``` 
Finally, in evaluating which program to implement the agent is assumed to be maximizing welfare measured using a _utility function_. The program outcomes mentioned above are in terms of an abstract quantity (e.g., income). A utility function means that there is a possibility of risk aversion and diminishing marginal utility: that it might be more optimal to increase the utility of those with a lower baseline utility (even if has relatively lower cost-effectiveness) or to implement programs with lower uncertainty. The utility function used is the _exponential utility_ function.

If there is uncertainty[Ignoring moral uncertainty.]{.aside} in the outcomes or there is variability in outcomes in the population, we would want to work with expected utilities. For example, given some means and standard deviations of outcomes over time, $\mu_{kt} + z\cdot \tau_{kt}$ and $\sigma_k$, respectively, the expected utility would be as in @fig-state-util-example. 
