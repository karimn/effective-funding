...
For each program $k$, we model the data generating process for each individual $i$'s outcome at step $t$ as,

$$
\begin{align*}
  Y_{it}(z) &\sim \mathtt{Normal}(\mu_{k[i],t} + z\cdot \tau_{k[i],t}, \sigma_{k[i]}) \\
  \\
...
where $z$ is a binary variable indicating whether a program is implemented or not. The hyperstate for each program $k$, $(\mu_k, \tau_k, \sigma_k, \eta^\mu_k, \eta^\tau_k)$, is drawn from the prior 
$$
\begin{align*}
...
\end{align*}
$$ 
where $\boldsymbol{\xi} = (\xi^\mu, \xi^\tau, \xi^\sigma, \xi^{\eta^\mu}, \xi^{\eta^\tau})$ are the hyperparameters for the environment. Thus we are modeling the counterfactual outcomes of program implementation as a hierarchical Bayesian model. 
...
$$
where $\alpha$ represents the degree of risk aversion.
...
#| fig-cap: "Population expected utility over time for 10 example programs. \\begin{align*}E[U(Y)] &= 1 - e^{-\\alpha\\mu + \\alpha^2 \\sigma^2/2}\\\\Y&\\sim\\mathtt{Normal}(\\mu,\\sigma)\\end{align*}."
#| fig-height: 6
#| cap-location: margin
