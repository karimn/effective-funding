...
vn_util_diff |>
  filter(accum) |> 
  filter(fct_match(plan_type, c("pftdpw", "freq", "random", "none"))) |> 
  ggplot(aes(step)) +
...
```{r, fig.width=10}
vn_util_diff |> 
  filter(!fct_match(plan_type, "best"), accum) |>
  ...
  tidybayes::stat_lineribbon(aes(linetype = "Median"), .width = 0.0, linewidth = 0.25, point_interval = median_qi) +
