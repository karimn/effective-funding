...
I model this as a sequence of decisions that need to be made at discrete intervals given a finite set of programs with uncertain impact upon a set of populations. Given some beliefs about the counterfactual outcomes of these programs for their targeted populations, the funder selects the optimal programs to implement and then decides what data to collect to update these beliefs, which in turn will be utilized at the next decision point. 

I intentionally keep the environment and the problem as simple as possible to keep it tractable.[Follow-up studies will iteratively revisit these assumptions.]{.aside} The funder's problem is treated as a bandit problem but without the restriction of only being able to evaluation implemented programs; I assume that each program targets a particular population without any kind of overlap; the cost of implementation is held fixed and equal for all programs; no new programs enter the problem over time; why the state of each program varies over time, it is drawn from a hierarchical and stationary program hyperstate.[_State_ here refers to the causal model determining outcome counterfactuals depending on whether a program is implemented or not. It is the data generating process from which we observed data when a program is evaluated.]{.aside} 

While the optimal method to select a program for implementation is a probabilistic one, taking account of the distribution of counterfactual quantities and any available prior information, I also consider the commonly used null hypothesis significance testing (NHST) approach.[^bayes-vs-freq] My purpose is not to compare the probabilistic and the NHST decision rules, but rather my focus is on the sequential nature of these decisions in the presence of heterogeneity in program effectiveness. I want to examine the potential to improve welfare by improving the planning scheme employed to select programs for re-evaluation. This is what I refer to as the _meta-problem_.

I am not trying to identify the optimal policy, only to consider the possibility of welfare improvement using alternative policies to what is conventionally used, keeping in mind that I am caricaturing those policies in service of tractability, ignoring some of their complexities and context specific adjustments and heuristics that expert decision-makers introduce. Nevertheless, I believe this study does capture the essence of how conventionally used policies underperform. 

Specifically, the policies that I want to show the limitations of are: (a) never re-evaluating programs and basing decisions solely on initial evaluations, (b) re-evaluating programs randomly, and (c) using NHST in a simple heuristic policy. Comparing those against policies using a POMDP[POMDPs are Partially Observable Markov Decision Processes.]{.aside} algorithm and a simple heuristic policy using a Bayesian hierarchical model. I find that the alternative policies are able to increase accumulated discounted utility by at least 20% after a few steps.

It is also worth mentioning that while the setup of the implementation-evaluation problem is inspired by the problem funding organizations of international development and global health charities, this setup is applicable to the broader decision problems of Effective Altruism. 
